...
if( $nv_Request->isset_request( 'submit', 'post' ) )
{
    ...
    $array['allowed_view'] = ! empty( $_groups_view ) ? implode( ',', nv_groups_post( array_intersect( $_groups_view, array_keys( $groups_list ) ) ) ) : '';
    ...
    else
    {
        if( $array['id'] == 0 )
        {
            $sql = "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . " VALUES (
				NULL, 
				" . $db->quote( $array['message'] ) . ", 
				" . $db->quote( $array['url'] ) . ", 
				" . $db->quote( $array['icon'] ) . ", 
                " . $db->quote( $array['author'] ) . ",
                " . intval( $admin_info['userid'] ) . ",
				" . $array['addtime'] . ", 
				" . intval( $array['showview'] ) . ",
				" . $db->quote( $array['allowed_view'] ) . ",
                " . intval( $array['status'] ) . ")";
            if( ! $db->query( $sql ) )
            {
        ...
        else
        {
            $sql = "UPDATE " . NV_PREFIXLANG . "_" . $module_data . " SET 
                ...
                showview=" . $array['showview'] . ",
                allowed_view=" . $db->quote( $array['allowed_view'] ) . ",
...
$array_notification_showview = array( 0 => $lang_module['notification_showview_0'], 1 => $lang_module['notification_showview_1'], 2 => $lang_module['notification_showview_2'] );
foreach( $array_notification_showview as $key => $title )
{
    $xtpl->assign( 'SHOWVIEW', array( //
        'key' => $key, //
        'title' => $title, //
        'selected' => ( $array['showview'] == $key ) ? " selected=\"selected\"" : "" ) );
    $xtpl->parse( 'main.showview' );
}

$allowed_view = explode( ',', $array['allowed_view'] );
